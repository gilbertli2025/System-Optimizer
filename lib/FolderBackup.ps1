...
function Get-UsbBackupFolders {
    ...
    @(Get-ChildItem $root -Directory -ErrorAction SilentlyContinue | Select-Object -ExpandProperty FullName)
}
...
function Backup-UserFolders {
    ...
    if (-not $Target) { $Target = Get-BackupBase; if (-not $Target) { Show-Message "No USB drive found to back up to.`n`nPlug in a USB drive and try again.`n`nClick OK to continue." 'Backup' Warn; return $false } }
...
function Export-InstalledPrograms {
    [CmdletBinding()]
    param([string]$Target)
    if (-not $Target) { $Target = Get-BackupBase; if (-not $Target) { Show-Message "No USB drive found.`n`nClick OK to continue." 'Export' Warn; return $null } }
    New-Item -ItemType Directory -Path $Target -Force | Out-Null
...
function Get-LastBackupInfo {
    [CmdletBinding()]
    param([string]$Target)
    if (-not $Target) { $Target = Get-BackupBase }
